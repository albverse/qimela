...
### J1. 设定与玩法
- 关卡中可搜集“回血精灵”道具。
- 玩家靠近精灵到一定范围后，精灵会自动飞向玩家并环绕玩家。
- 玩家最多携带 **2 只**回血精灵。
- 按下 **C** 使用回血精灵，消耗 **1 只**。
- 每只回血精灵回复玩家 **2.5 颗红心**。

### J2. 视觉对标与运动要求（对标《GRIS》）
- 精灵环绕效果需对标《GRIS》女主身边光球的轨迹与观感。
- 精灵飞行轨迹需与玩家位移“松耦合”，禁止表现为绑定节点的瞬移：
  - 玩家跳跃时，精灵应有 **约 1 秒延迟**，并逐步加速追上玩家位置。
  - 玩家快速奔跑到别处时，精灵应在 **数秒内逐渐加速** 追上玩家位置。
- 精灵追上玩家后，继续以《GRIS》风格稳定环绕。
- 两只精灵必须错开相位/轨道，避免重叠（视觉不好看）。

### J3. 实现建议（方案）
**方案 A：纯 Godot（推荐）**
- 状态机：`IdleInWorld`（待机）→ `Acquire`（吸附追踪）→ `Orbit`（稳定环绕）→ `Consumed`（消耗）。
- 追踪模型：目标点追踪 + 速度上限 + 加速度递增；在 `Acquire` 阶段加入延迟与惯性，避免“绑点”感。
- 环绕阶段：按角速度更新 `angle`，叠加相位偏移与半径差异，确保多只精灵错位。

**方案 B：Spine2D + Godot**
- Spine2D 用于精灵本体漂浮/脉动动画；Godot 控制轨迹与追踪逻辑。
- 保持轨迹“拖尾感/惯性感”，对标《GRIS》观感。
