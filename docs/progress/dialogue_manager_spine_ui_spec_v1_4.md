...
var skin_override: StringName
var bubble_style_override: StringName
...
- ✅ 已采用 `DialogueRunner` + `DialogueStage` + 控制器分层结构。
  - `DialogueRunner` 负责取行、起停、衔接下一句。
  - `DialogueStage` 负责 UI/输入门控/响应选项显示与舞台调度。
...
- 🔄 输入职责从“Runner 拦截”调整为“Stage 拦截 + Runner 仅驱动流程”，用于更好贴合舞台 UI 逻辑。

### 17.2 UI 槽位系统（AB 历史 / CD 当前）对标

- ✅ 四槽位逻辑已实现并可在编辑器拖动 Marker2D 调整。
- ✅ CD 区同一时刻仅一个当前气泡；下一句出现时上一句迁移到历史区（A/B），再下一句时旧历史淡出。
- 🆕 气泡与槽位的对齐方式改为“**以槽位为中心**对齐气泡（非左上角对齐）”，便于美术精确摆位。
- ⚠️ 历史态布局仍基于当前气泡实例转换（符合现有演出需求）；若后续需要“历史态独立 prefab”，可再扩展。

### 17.3 responses 分支能力对标

- 🆕 已新增 `ResponsesLayer/ResponsesContainer`，可在对话行存在 responses 时动态生成按钮。
- 🆕 已新增 `dialogue_response_selected(next_id)` 信号，选择后回写到流程推进。
- ✅ 分支选择后由 Runner 按 `next_id` 继续 `get_next_dialogue_line(...)`。
- 🆕 已补“responses 显示期间继续吞输入”逻辑，避免玩家输入泄漏到主游戏控制。

### 17.4 输入策略对标（只允许左键推进）

- ✅ 对话期间仅左键用于推进（非 responses 场景）。
- ✅ 键盘输入不用于推进并被吞掉。
- ✅ 对话激活态通过 `set_dialogue_active(true/false)` 显式门控。

### 17.5 立绘系统对标（独立场景、入场动画、可调参数）

- ✅ `SpinePortrait` 作为独立场景，支持独立实例化与参数调节。
- ✅ 入场滑入效果已实现：支持 `offset_x / duration / ease / trans`，满足“贝塞尔缓入缓出可调”需求。
- ✅ player 与 other 可配置相反方向滑入（右侧正偏移、左侧负偏移）。
- ✅ 立绘默认不做自动翻转逻辑（由资源/摆位控制视觉朝向）。

### 17.6 皮肤同步策略对标（美术素材未就绪阶段）

- ✅ 当前默认可关闭 `player_skin_sync_enabled` 与 `light_state_sync_enabled`，便于先行联调演出与文本流程。
...
- ✅ 支持运行时设置气泡纹理与材质，满足后续 shader/动效扩展。
- ✅ 结构兼容 `NinePatchRect` / `TextureRect`，便于美术更换实现路径。

### 17.8 可复用基架对标（减少测试场景重复搭建）

- 🆕 `DialogueTestScene` 已改为实例化标准 `DialogueStage.tscn` 与标准 `SpinePortrait.tscn`，降低重复搭建成本。
- 🆕 测试场景新增测试骨骼资源注入逻辑，确保实例化标准立绘后仍可快速验证。
- ✅ 后续新测试场景可沿用该装配方式，避免复制粘贴整套舞台节点树。

### 17.9 生命周期与稳定性修正对标

- 🆕 `SpinePortrait.setup_controller()` 增加幂等保护，避免重复创建控制器。
- 🆕 增加 `reset_controller()` 并在对话结束时调用，防止重开对话累积无效控制器。
- 🆕 `stop_talk()` 增加 `talk_finished` 事件发出，便于后续系统订阅状态切换。

### 17.10 当前状态总览（便于项目管理）

- 已实现主路径：文本推进、AB/CD 槽位、历史迁移、打字机、立绘入场、responses 选择。
- 已完成架构收敛：Stage 负责输入与演出，Runner 负责流程推进。
- 已完成美术友好化：关键位置、动效参数、样式材质可在编辑器调整。
- 后续可选增强项（非阻塞）：
  1. 历史态专用 prefab 与布局策略（进一步强化可控性）；
  2. 皮肤/亮暗同步启用后的项目联调规范；
  3. 更细粒度的 responses 焦点导航与手柄支持（若有主机需求）。

真正把它变成图中那种黑暗奇幻双侧气泡 + Spine 演出的，是你自己的舞台层。
