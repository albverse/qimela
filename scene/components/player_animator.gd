	...
	if track == TRACK_LOCO:
		...
		if _cur_action_mode == MODE_FULLBODY_EXCLUSIVE and _cur_action_anim == anim_name:
			...
			if anim_name == &"chain_/die" or anim_name == &"ghost_fist_/die":
				return
			# GF attack 动画完成由 GhostFist 自身完成回调驱动，PlayerSpine 不抢跑清状态
			...
			if anim_name == &"ghost_fist_/hurt" and _gf_mode and _ghost_fist != null and _ghost_fist.state == GhostFist.GFState.GF_COOLDOWN:
				play_ghost_fist_cooldown()
