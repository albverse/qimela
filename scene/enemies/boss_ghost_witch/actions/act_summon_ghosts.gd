...
## Phase 3 召唤幽灵：施法起手（含 combo3 攻击）→ 生成幽灵波次 → summon_loop 维持

enum Step { CAST, SUMMON_LOOP }
...
func tick(actor: Node, blackboard: Blackboard) -> int:
	...
	match _step:
		Step.CAST:
			...
			_wave_timer += dt
			var expected_waves: int = int(_wave_timer / _wave_interval)
...
func _spawn_wave(boss: BossGhostWitch) -> void:
	...
	for i: int in range(boss.p3_summon_circle_count - 1):
		var random_x: float = player.global_position.x + randf_range(-300, 300)
		positions.append(Vector2(random_x, player.global_position.y))
	for pos: Vector2 in positions:
		var summon: Node2D = (boss._ghost_summon_scene as PackedScene).instantiate()
