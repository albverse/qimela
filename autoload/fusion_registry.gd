...
func _execute_fail_hostile(result: Dictionary, player: Player) -> Node:
	...
	var entity_a: EntityBase = result.get("entity_a") as EntityBase
	var entity_b: EntityBase = result.get("entity_b") as EntityBase
	var hostile_scene_path: String = result.get("hostile_scene", "")
	...
	if hostile_scene_path.is_empty() or not ResourceLoader.exists(hostile_scene_path):
		push_error("[FusionRegistry] hostile_scene 无效: %s" % hostile_scene_path)
		return null

	var hostile_scene: PackedScene = load(hostile_scene_path) as PackedScene
	if hostile_scene == null:
		push_error("[FusionRegistry] hostile_scene 加载失败: %s" % hostile_scene_path)
		return null

	var hostile: Node = hostile_scene.instantiate()
	if hostile is Node2D:
